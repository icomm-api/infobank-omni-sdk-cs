...
{
    public class BrandMessageService : MessageService, IMessageService
    {
        private readonly string _brandMessageSvcUrl;
        ...
        public BrandMessageService(HttpClient client, string baseUrl, string token, ILogger logger) : base(client, baseUrl)
        {
            _brandMessageSvcUrl = "v1/send/brandmessage";

            _token = token;
            _logger = logger;
            _typeName = "BrandMessageService";
        ...
        {
            if (message is BrandMessageRequest request)
            {
            ...
            else
            {
                ...
                _logger.LogInformation("[{Type}] Not vaild BrandMessage Request data", _typeName);
            }
            return "";
        }
        ...
        public ApiResponse? SendRequest<T>(T message)
        {
            try
            {
                ...
                HttpResponseMessage response = _client.PostAsync(_baseUrl + _brandMessageSvcUrl, content).Result;
                ...
                {
                    _logger.LogInformation("[{Type}] RequestFailed code:{code} url: {url}", _typeName, response.StatusCode, _baseUrl + _brandMessageSvcUrl);
