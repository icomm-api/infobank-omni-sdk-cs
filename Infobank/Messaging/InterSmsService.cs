...
{
    public class InterSmsService : MessageService, IMessageService
    {
        private readonly string _interSmsSvcUrl;
        ...
        public InterSmsService(HttpClient client, string baseUrl, string token, ILogger logger) : base(client, baseUrl)
        {
            _interSmsSvcUrl = "v1/send/international";

            _token = token;
            _logger = logger;
            _typeName = "InterSmsService";
        }
        ...
        {
            if (message is InterSmsRequest request)
            {
            ...
            {
                _logger.LogInformation("[{Type}] Not vaild International SMS Request data", _typeName);
            }
            return "";
        }
        ...
        public ApiResponse? SendRequest<T>(T message)
        {
            try
            {
                ...
                HttpResponseMessage response = _client.PostAsync(_baseUrl + _interSmsSvcUrl, content).Result;
                ...
                {
                    _logger.LogInformation("[{Type}] RequestFailed code:{code} url: {url}",_typeName, response.StatusCode, _baseUrl + _interSmsSvcUrl);
