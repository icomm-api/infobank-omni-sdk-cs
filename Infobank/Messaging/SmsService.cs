...
{
    public class SmsService : MessageService, IMessageService
    {
        private readonly string _smsSvcUrl;
        ...
        public SmsService(HttpClient client, string baseUrl, string token, ILogger logger) : base(client, baseUrl)
        {
            _smsSvcUrl = "v1/send/sms";
            _token = token;
            _logger = logger;
            _typeName = "SmsService";
        ...
        {
            if (message is SmsRequest request)
            {
            ...
            else
            {
                ...
                _logger.LogInformation("[{Type}] Not vaild SMS Request data", _typeName);
            }
            return "";
        }
        ...
        public ApiResponse? SendRequest<T>(T message)
        {
            try
            {
                ...
                HttpResponseMessage response = _client.PostAsync(_baseUrl + _smsSvcUrl, content).Result;
                ...
                {
                    _logger.LogInformation("[{Type}] RequestFailed code:{code} url: {url}", _typeName, response.StatusCode, _baseUrl + _smsSvcUrl);
